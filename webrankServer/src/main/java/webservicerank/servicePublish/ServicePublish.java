...
public class ServicePublish {
	 ...
	 * @throws Throwable
	 */
	public boolean[] validate(String wsdlUrl, String serviceName, long[] serviceDelay, int[] invoke_counts)
			throws Throwable {
		...
		try {
			isValid[0] = !publishDB.isAreadyStored(wsdlUrl, serviceName); // False
																			// if
																			// DB
																			// contains
																			// the
		...
		publishDB = new PublishDB();
		serviceDelay[0] = 0;
		isValid[1] = validator.validate(serviceDelay, invoke_counts); // False
																		// if
																		// the
																		// service
																		// is
	...
	// } catch (Throwable ex) {
	// Logger.getLogger(ServicePublish.class.getName()).log(Level.SEVERE, null,
	// ex);
	// }
