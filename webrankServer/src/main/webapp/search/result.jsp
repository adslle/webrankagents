	...
	<div id="wrapper">
		...
		<div id="header">
			...
			<div style="padding-top: 110px; padding-left: 800px;">
				<label><h2>Rank By:</h2></label> <select id="where"
					name="SearchFormAction" onchange="rankBy(this.value)">
						...
						<table cellspacing="0">
                         ...
                         var resultdata = '<%=responseText%>';
								...
								if (values[0] == 'Error' && values.length == 2) {
									htmlText = values[1]
											+ ' <a href="../search/index.jsp">Search again</a>';
								} else {
									var length = values.length - 1;
									document.getElementById('service_count').innerHTML = length
											+ " services matched";
									var rank = 1;
									for (var i = 0; i < values.length - 1; i++) {
										...
										if (row[2] == '2')
											...
											tempText = "Not rated";
										htmlText += "<tr onmouseout=\"this.bgColor=''\" onmouseover=\"this.bgColor='lightblue'\"><td style=\"color:black\">"
												+ rank++
												+ "</td><td style=\"color:darkblue\">"
												+ row[0]
												+ "\
                                           </td><td>"
												+ row[1]
												+ "</td><td width=\"80px\">"
												+ tempText
												+ "</td><td><a href=\"../SearchInterface?service_id="
												+ row[3]
												+ "&rank="
												+ i
												+ "\">Call it!</a></td></tr>";
