...
function check_service_name(servicename, flag) {
	...
	if (count > 0) {
		...
		if (flag == 1)
			...
			error_message(9,
					"The Precondition properties contains space.Its invalid");
		else if (flag == 10)
			error_message(10,
					"The Effect properties contains space.Its invalid");
	}
}
...
function check_URL_pattern(val, URL) {
	...
	if (URL != ""
			&& !URL
					.match(/^http[s]*\:\/\/[wW]{3}\.+[a-zA-Z0-9]+\.[a-zA-Z]{2,3}.*$|^http[s]*\:\/\/[^w]{3}[a-zA-Z0-9]+\.[a-zA-Z]{2,3}.*$|http[s]*\:\/\/[0-9]{2,3}\.[0-9]{2,3}\.[0-9]{2,3}\.[0-9]{2,3}.*$/)) {
		isfieldvalid = false;
		if (val == 4)
